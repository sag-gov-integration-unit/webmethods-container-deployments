...
--image "$REGISTRYSERVERNAMEwebmethods-sample-apis-bookstore:$TAG_SAMPLE_APIS" -n webmethods-apis-bookstore \
--dns-name-label $DNS_PREFIXapis-bookstore --cpu 1 --memory 1 --restart-policy Always --ports 7071

echo Deploying Sample US Zip API...
az container create --no-wait --resource-group "$RESOURCEGROUP" --registry-login-server "$REGISTRYLOGINSERVER" \
--registry-password "$REGISTRYPASSWORD" --registry-username "$REGISTRYUSERNAME" \
--image "$REGISTRYSERVERNAMEwebmethods-sample-apis-uszip:$TAG_SAMPLE_APIS" -n webmethods-apis-uszip \
--dns-name-label $DNS_PREFIXapis-uszip --cpu 1 --memory 1 --restart-policy Always --ports 7071
...
az container create --no-wait --resource-group "$RESOURCEGROUP" --registry-login-server "$REGISTRYLOGINSERVER" \
--registry-password "$REGISTRYPASSWORD" --registry-username "$REGISTRYUSERNAME" \
--image "$REGISTRYSERVERNAMEwebmethods-apiportal:$TAG_APIPORTAL" -n webmethods-apiportal \
--dns-name-label $DNS_PREFIXapiportal --cpu 2 --memory 8 --restart-policy Always --ports 18101 18102 \
