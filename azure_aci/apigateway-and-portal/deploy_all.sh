...
--image "$REGISTRYLOGINSERVERwebmethods-sample-apis-bookstore:$TAG_SAMPLE_APIS" -n webmethods-apis-bookstore \
--dns-name-label apis-bookstore --cpu 1 --memory 1 --restart-policy Always --ports 7071

echo Deploying Sample US Zip API...
az container create --no-wait --resource-group "$RESOURCEGROUP" --registry-login-server "$REGISTRYLOGINSERVER" \
--registry-password "$REGISTRYPASSWORD" --registry-username "$REGISTRYUSERNAME" \
--image "$REGISTRYLOGINSERVERwebmethods-sample-apis-uszip:$TAG_SAMPLE_APIS" -n webmethods-apis-uszip \
--dns-name-label apis-uszip --cpu 1 --memory 1 --restart-policy Always --ports 7071

echo Deploying API Gateway...
az container create --no-wait --resource-group "$RESOURCEGROUP" --registry-login-server "$REGISTRYLOGINSERVER" \
--registry-password "$REGISTRYPASSWORD" --registry-username "$REGISTRYUSERNAME" \
--image "$REGISTRYLOGINSERVERwebmethods-apigateway-standalone:$TAG_APIGATEWAY" -n webmethods-apigateway \
...
az container create --no-wait --resource-group "$RESOURCEGROUP" --registry-login-server "$REGISTRYLOGINSERVER" \
--registry-password "$REGISTRYPASSWORD" --registry-username "$REGISTRYUSERNAME" \
--image "$REGISTRYLOGINSERVERwebmethods-apigateway-configurator:$TAG_APIGATEWAY_CONFIGURATOR" -n webmethods-gw-config \
...
az container create --no-wait --resource-group "$RESOURCEGROUP" --registry-login-server "$REGISTRYLOGINSERVER" \
--registry-password "$REGISTRYPASSWORD" --registry-username "$REGISTRYUSERNAME" \
--image "$REGISTRYLOGINSERVERwebmethods-apiportal:$TAG_APIPORTAL" -n webmethods-apiportal \
